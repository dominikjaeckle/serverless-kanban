...
// jwks key for verification
const jwksUrl = `https://${process.env.AUTHID}.eu.auth0.com/.well-known/jwks.json`;
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  ...
  const signingKey = await getSigningKey(jwksUrl, jwt.header.kid);
...
async function getSigningKey(jwkurl, kid) {
